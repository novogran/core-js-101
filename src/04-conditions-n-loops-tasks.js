...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(char) === i
      && str.indexOf(char, i + 1) === -1) return char;
  }
  return null;
}
...
function getDigitalRoot(num) {
  let result = num.toString().split('').reduce((sum, value) => sum + parseInt(value, 10), 0);
  while (result > 9) {
    result = result.toString().split('').reduce((sum, value) => sum + parseInt(value, 10), 0);
  }
  return result;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i <= result.length + 2; i += 1) {
    for (let k = 0; k < bracketsConfig.length; k += 1) {
      while (result.includes(bracketsConfig[k][0].toString() + bracketsConfig[k][1].toString())) {
        result = result.replaceAll(bracketsConfig[k][0].toString() + bracketsConfig[k][1].toString(), '');
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  // let result = pathes[0].charAt(0);
  // let flag = true;
  // for (let k = 1; k < pathes.length; k += 1) {
  //   while (flag) {
  //     for (let i = 0; i < pathes[0].length; i += 1) {
  //       if (pathes[k].includes(result)) {
  //         result += pathes[0].charAt(i + 1);
  //       } else {
  //         flag = false;
  //       }
  //     }
  ...
  // }
  // return result;
  const item = pathes[0].split('/').filter((value) => value !== '');
  let result = '';
  let counter = 0;
  let counterStart = 0;
  for (let i = 0; i < item.length; i += 1) {
    for (let k = 0; k < pathes.length; k += 1) {
      if (pathes[k].indexOf('/') === 0) counterStart += 1;
      if (pathes[k].split('/').filter((value) => value !== '').includes(item[i])) counter += 1;
    }
    if (counterStart === pathes.length) result = '/';
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1Rows; i += 1) {
    ...
    for (let k = 0; k < m2Cols; k += 1) {
      result[i][k] = 0;
      for (let r = 0; r < m1Cols; r += 1) {
        result[i][k] += m1[i][r] * m2[r][k];
      }
    }
  }
  return result;
